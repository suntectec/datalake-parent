...
public class DataGenerator2MinIOS3 {
    public static void main(String[] args) throws Exception {
        // 初始化 s3 插件
        Configuration conf = new Configuration();
        conf.setString("fs.allowed-fallback-filesystems", "s3");
        conf.setString("s3a.endpoint", "http://192.168.138.15:9000");
        conf.setBoolean("s3a.path.style.access", true);
        conf.setString("s3a.access-key", "minioadmin");
        conf.setString("s3a.secret-key", "minioadmin");
        ...
        FileSink<String> sink = FileSink
                .forRowFormat(new Path("s3a://warehouse/test/flink/sink"), new SimpleStringEncoder<String>("UTF-8"))
                // 基于 Checkpoint 的滚动策略
